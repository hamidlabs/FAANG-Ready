...
export default function ContentWithNotesImproved({ content, lessonId }: ContentWithNotesProps) {
    ...
    sortedNotes.forEach(note => {
      ...
      for (const textNode of textNodes) {
        ...
        if (index !== -1) {
          ...
          span.className = 'relative bg-yellow-100/20 border-b-2 border-yellow-400/50 cursor-pointer hover:bg-yellow-100/30 transition-colors';
          span.title = `Note: ${note.note_content.substring(0, 100)}${note.note_content.length > 100 ? '...' : ''}`;
          ...
          indicator.className = 'absolute -top-2 -right-2 w-4 h-4 bg-yellow-500 text-white text-xs rounded-full flex items-center justify-center hover:bg-yellow-600 transition-colors z-10';
          indicator.innerHTML = '📝';
  ...
  const components = {
    ...
    p: ({children}: any) => <p className="mb-6 text-gray-300 leading-relaxed text-lg">{children}</p>,
